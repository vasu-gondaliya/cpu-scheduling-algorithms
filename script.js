...
document.querySelector(".calculate").onclick = () => {
  ...
  algorithmArray.forEach(function (element) {
    ...
    avgrt /= process;
    algorithmChartData.push([element, avgct, avgtat, avgwt, avgrt]);
    resetVariables();
  });
  ...
  output.appendChild(ganttChartHeading);
  ...
  output.appendChild(timelineChart);
...
function fcfs() {
  while (done.some((element) => element == false)) {
    let candidates = processId.filter(
      (element) => done[element] == false && returnTime[element] <= currentTime
    );
    if (candidates.length == 0) {
      ...
      schedule.push([
        found + 1,
        processTimeArray[found][currentProcessIndex[found]],
      ]);
...
function rr() {
  ...
  while (processQueue.length == 0) {
    currentTime++;
    schedule.push([-1, 1]);
    processQueue = processId.filter(
      (element) => returnTime[element] <= currentTime
    );
  }
    ...
    if (processQueue.length == 0) {
      ...
      processId.forEach(function (element) {
        if (
          ...
          returnTime[element] <= currentTime
        ) {
